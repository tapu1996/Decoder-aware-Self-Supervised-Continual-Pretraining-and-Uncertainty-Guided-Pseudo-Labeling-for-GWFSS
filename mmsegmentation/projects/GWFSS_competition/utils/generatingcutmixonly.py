...
out_dir = '/home/tapotosh/projects/def-farma/tapotosh/DenseSSL_root/mmsegmentation/projects/GWFSS_competition/data/gwfss_augmented_less'
...
for domain, file_list in image_groups.items():
    ...
    domain_img_out = os.path.join(out_dir, 'images')
    domain_mask_out = os.path.join(out_dir, 'class_id')
    os.makedirs(domain_img_out, exist_ok=True)
    os.makedirs(domain_mask_out, exist_ok=True)

    total_aug = len(file_list) * 2  # 2 CutMix + 2 ClassMix per original image
    aug_idx = 0
