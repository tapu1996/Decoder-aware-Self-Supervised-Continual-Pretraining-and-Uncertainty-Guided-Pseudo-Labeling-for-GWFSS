...
for image_file in image_files:
    ...
    for model in models:
        ...
        print("Original image shape:", orig_img.shape)  # (H, W, 3)

        predicted_mask = result.pred_sem_seg.data.cpu().numpy()  # shape: (H, W)
        print("Predicted mask shape:", predicted_mask.shape)

        logits = result.seg_logits.data  # shape: (num_classes, H, W)
        print("Segmentation logits shape:", logits.shape)
        exit(0)
        print("Input image shape to model:", result.inputs.shape)  # (1, 3, H, W)
        print("Segmentation logits shape:", result.seg_logits.shape)  # (num_classes, H, W)
        exit()
        logits = result.seg_logits.data  # Shape: (num_classes, H, W)
